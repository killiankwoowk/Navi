...
export const RecentlyPlayed = () => {
  useDocumentTitle('Recently Played | Navi Terminal Player')
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      return client.getRecentSongs(80)
    },
    staleTime: 5 * 60_000,
  })
  ...
  return (
    <TerminalPanel title="Recently Played">
      {!query.isLoading && tracks.length === 0 ? (
        <EmptyState title="No recent plays yet." hint="Play tracks to build your history feed." />
      ) : (
        <PlaylistView
          title={query.data?.length ? 'Server Recent Songs' : 'Local Usage Fallback'}
          tracks={tracks}
